...
let translate_file file_path syntax_map syntax_en_map output_folder_opt =
  try
    let ic = open_in file_path in
    let output_file = match output_folder_opt with
      | Some output_folder -> Filename.concat output_folder (Filename.basename (file_path ^ ".snt"))
      | None -> (Filename.remove_extension file_path) ^ ".snt"
    in
